...
for i:=0 to Length(Points) -1 do
   begin
  	if (Points[i].x > viewPosition.x + Extent) then
  	Points[i].x -= size;

        if (Points[i].x < viewPosition.x - Extent) then
  		Points[i].x += size;

  	if (Points[i].y > viewPosition.y + Extent) then
  		Points[i].y -= size;

        if (Points[i].y < viewPosition.y - Extent) then
  		Points[i].y += size;

  	if (Points[i].z > viewPosition.z + Extent) then
  		Points[i].z -= size;
	 ...
	 farLerp := Clamp(Normalize(distance, Extent * 0.9, Extent), 0, 1);
	  ...
	  farAlpha := round(Lerp(255, 0, farLerp));
	   ...
	   cubeSize := 0.01;
		...
		DrawLine3D(
			Vector3Add(Points[i], Vector3Scale(velocity, 0.01)),
			Points[i], ColorCreate
			( Colors[i].r, Colors[i].g, Colors[i].b, farAlpha ));
	end;
